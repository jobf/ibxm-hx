...
HL_PRIM int HL_NAME(initialise)(vbyte *file_data, int file_length,
  ...
  if (replay) {
    ...
    printf("Unable to initialise replay\n");
  }
...
HL_PRIM void HL_NAME(get_sample_data)(int instrument, int sample, vbyte *output_buffer) {
...
HL_PRIM bool HL_NAME(is_muted)(int channel) {
...
HL_PRIM int HL_NAME(get_num_patterns)(_NO_ARG) {
  return module->num_patterns;
}
...
HL_PRIM int HL_NAME(get_num_channels)(_NO_ARG) {
...
HL_PRIM int HL_NAME(get_row)(_NO_ARG) {
  return replay_get_row(replay);
}
...
// get_pattern_data — copies num_channels * num_rows * 5 bytes into output_buffer
